...
class Solution {
    public int getNumberOfBacklogOrders(int[][] orders) {
        Queue<int[]> buys = new PriorityQueue<>((a, b) -> b[0] - a[0]), sells = new PriorityQueue<>((a, b) -> a[0] - b[0]);
        for (int[] order : orders) {
            if (order[2] == 0) {
                while (order[1] > 0 && !sells.isEmpty()) {
                    int[] tmp = sells.poll();
                    if (order[0] >= tmp[0]) {
                        int min = Math.min(order[1], tmp[1]);
                        order[1] -= min;
                        tmp[1] -= min;
                        if (tmp[1] > 0)
                            sells.add(tmp);
                ...
                if (order[1] > 0)
                    buys.add(new int[]{order[0], order[1]});
            } else {
                while (order[1] > 0 && !buys.isEmpty()) {
                    int[] tmp = buys.poll();
                    if (order[0] <= tmp[0]) {
                        int min = Math.min(order[1], tmp[1]);
                        order[1] -= min;
                ...
                if (order[1] > 0)
                    sells.add(new int[]{order[0], order[1]});
            }
        }
        long MAX = 1000000007;
        long ans = 0;
